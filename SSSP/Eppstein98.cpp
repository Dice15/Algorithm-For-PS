...
public:   
	...
	void ConstructPersistentHeap() {
		...
		for (auto& vertex : mVisit) {
			...
			for (auto& edge : mGraph[vertex]) {
				...
				datatype sidetrack = edge.weight + mDist[edge.next] - mDist[vertex];   // sidetrack 가중치 = sidetrack(u, v, w) =  w + d[v] - d[u];
	...
	vector<datatype> Ksp(int source, int sink, int kValue) {
		...
		if (mDist[mSource] > BIG) return vector<datatype>(kValue, -1);  // 최단경로가 1개도 없는 경우
